...
export class PostgresUserService implements IServicelocator {
  ...
  async findAllUserDetails(userSearchDto, tenantId?: string) {
    ...
    if (tenantId && tenantId.trim() !== "") {
      ...
      LoggerUtil.log(
        `Applying tenant filter for tenantId: ${tenantId}`,
        APIID.USER_LIST
      );
    } else {
      LoggerUtil.warn(
        `No tenantId provided - returning users from all tenants`,
        APIID.USER_LIST
      );
    }
    ...
    try {
      ...
      if (reason === "signup" || reason === "login") {
        ...
        if (channelOverride === "sms") {
          ...
          notificationPayload = await this.smsNotification(
            "OTP",
            "SEND_OTP",
            replacements,
            [mobile]
        ...
        if (reason == "forgot" && body.email) {
          const result = await this.sendOtpOnMailForgot(body);
        } else {
    ...
    try {
      ...
      if (reason === "signup" || reason === "login") {
        try {
          ...
          LoggerUtil.warn(
            `WhatsApp OTP send failed: ${waErr?.message || waErr}`,
            APIID.SEND_OTP
          );
        }
      }
  ...
  async sendOtpOnWhatsApp(whatsapp: string, otp: string, reason: string) {
    try {
      ...
      const notificationPayload = await this.whatsappNotificationRaw(
        whatsapp,
        otp,
        reason
      );
  ...
  async whatsappNotification(
    ...
    try {
      ...
      if (
        result?.result?.whatsapp?.errors &&
        result.result.whatsapp.errors.length > 0
      ) {
        const errorMessages = result.result.whatsapp.errors.map(
          (error: { error: string }) => error.error
        );
        const combinedErrorMessage = errorMessages.join(", ");
        throw new Error(
            ...
            if (usernameToLookup) {
              ...
              if (tokens?.access_token) {
                ...
                if (tokens?.expires_in)
                  responseData["expires_in"] = tokens.expires_in;
                if (tokens?.refresh_expires_in)
                  responseData["refresh_expires_in"] =
                    tokens.refresh_expires_in;
                if (tokens?.token_type)
                  responseData["token_type"] = tokens.token_type;
              }
            }
          } catch (ex) {
            // Non-blocking: OTP success even if token exchange fails
            LoggerUtil.warn(
              `Keycloak token exchange failed: ${ex?.message || ex}`,
              APIID.VERIFY_OTP
            );
          }
        }
  ...
  async sendOtpOnMailForgot(body) {
