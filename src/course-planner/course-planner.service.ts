...
export class CoursePlannerService {
  ...
  constructor(
    ...
    this.internalAccessToken = this.configService.get<string>(
  ...
  async processUploadedData(
    ...
    try {
      ...
      const checkAndCreateProject = await this.validateAndCreateProject(
        projectExternalId,
        metaData,
        createProjectDto
      );
      ...
      const createSolution = await this.createSolution(
        programData?.result?.externalId,
        metaData
      );
      const result = await this.mappingSolution(
        projectExternalId,
        createSolution?.result?._id
      );
      ...
      this.logger.error(
        `Error in processUploadedData: ${error.message}`,
        error.stack
      );
  ...
  private generateProjectExternalId(metaData: MetaDataDto): string {
    return `${metaData.subject.slice(0, 2)}${metaData.state.slice(
      0,
      2
  ...
  private async createSolution(
    ...
    try {
      ...
      this.logger.error(
        `Error creating solution: ${error.message}`,
        error.stack
      );
      throw new HttpException(
  ...
  private prepareSolutionData(externalId: string, metadata: MetaDataDto): any {
    ...
    return {
      ...
      startDate: "2024-09-25 18:50:00",
      endDate: "2025-10-19 18:50:00",
    };
  }

  private async mappingSolution(
    externalId: string,
    solutionId: string
  ): Promise<any> {
    try {
      ...
      this.logger.error(
        `Error mapping solution: ${error.message}`,
        error.stack
      );
      throw new HttpException(
        "Failed to map solution",
        HttpStatus.INTERNAL_SERVER_ERROR
      );
    }
  }
  ...
  //   }
  // }
  private async createProjectSubtask(
    projectId: string,
    file: Express.Multer.File
  ): Promise<any> {
    try {
      ...
      if (
        responseData.includes("Project template tasks already exists") ||
        responseData.includes('"Project template tasks already exists"')
      ) {
        throw new HttpException(
          "Project Template Already Exists. Make Sure you add unique exteranl Id",
          HttpStatus.BAD_REQUEST
        );
      }

      this.logger.log(
        `Project subtask Created Successfully: ${JSON.stringify(response.data)}`
      );
      return response.data;
    } catch (error) {
      this.logger.error(
        `Error creating project subtask: ${error.message}`,
        error.stack
      );
      if (error instanceof HttpException) {
      ...
      throw new HttpException(
        "Failed to create project subtask",
        HttpStatus.INTERNAL_SERVER_ERROR
      );
    }
  }

  private async validateAndCreateProject(
    externalId: string,
    metaData: MetaDataDto,
    createProjectDto: CreateProjectDto
  ) {
    try {
      ...
      this.logger.error(
        `Error validating project: ${error.message}`,
        error.stack
      );
      throw new HttpException(
  ...
  private async createProject(
    ...
    try {
      const csvBuffer = await this.createCsvFromDto(
        externalId,
        metaData,
        createProjectDto
      );
  ...
  private async uploadProjectTemplate(csvBuffer: Buffer): Promise<any> {
    try {
      ...
      this.logger.error(
        `Error uploading project template: ${error.message}`,
        error.stack
      );
      throw new HttpException(
        "Failed to upload project template",
        HttpStatus.INTERNAL_SERVER_ERROR
      );
    }
  }

  // private async createCsvFromDto(
  //   externalId: string,
  //   metaData: MetaDataDto,
  //   createProjectDto: CreateProjectDto
  // ): Promise<Buffer> {
  ...
  private async createCsvFromDto(
    ...
    return new Promise((resolve, reject) => {
      ...
      stringify([headers, values], (err, output) => {
        ...
        // Write the file
        fs.writeFile("output.csv", buffer, (writeErr) => {
          if (writeErr) {
            this.logger.error(
              `Error writing CSV file: ${writeErr.message}`,
              writeErr.stack
            );
            return reject(
              new HttpException(
  ...
  private prepareCsvData(
    ...
    metaData: MetaDataDto,
    createProjectDto: CreateProjectDto
  ): Array<{ column1: string; column2: string }> {
    return [
      { column1: "title", column2: metaData.subject },
      { column1: "externalId", column2: externalId },
      { column1: "categories", column2: "educationLeader" },
      {
        column1: "recommendedFor",
        column2:
          "Head Master/Mistress (HM),Principal,Head Teacher,Elementary School Head Master (ESHM)",
      },
