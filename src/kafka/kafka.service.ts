...
export class KafkaService implements OnModuleInit, OnModuleDestroy {
  ...
  private readonly TYPE_OF_CENTER_FIELD_ID =
  ...
  private extractTypeOfCenter(customFields: any[]): string | null {
    ...
    if (
      typeOfCenterField.selectedValues &&
      Array.isArray(typeOfCenterField.selectedValues)
    ) {
  ...
  private async queryTypeOfCenter(cohortId: string): Promise<string | null> {
    try {
      ...
      this.logger.error(
        `Failed to query TYPE_OF_CENTER for cohort ${cohortId}: ${error.message}`
      );
      return null;
    }
  }
   ...
   */
  private transformCohortType(
    cohortType: "COHORT" | "BATCH",
    typeOfCenter: string | null
  ): string {
    if (
      !typeOfCenter ||
      !(cohortType in this.TYPE_MAPPINGS) ||
      !(typeOfCenter in this.TYPE_MAPPINGS[cohortType])
    ) {
  ...
  async publishUserTenantEvent(
    ...
    if (!this.isKafkaEnabled) {
      this.logger.warn(
        "Kafka is disabled. Skipping user-tenant event publish."
      );
      return; // Do nothing if Kafka is disabled
    }
    ...
    await this.publishMessage(topic, payload, userId);
    this.logger.log(
      `User-tenant ${eventType} event published for user ${userId}`
    );
  }
  ...
  async publishCohortEvent(
    ...
    try {
      ...
      if (cohortType === "COHORT") {
        ...
        this.logger.debug(
          `COHORT event - customFields structure:`,
          JSON.stringify(cohortData?.customFields || [], null, 2)
        );
        typeOfCenter = this.extractTypeOfCenter(cohortData?.customFields || []);
        this.logger.log(
          `COHORT event - TYPE_OF_CENTER extracted: ${typeOfCenter}`
        );
        ...
        if (parentCohortId) {
          ...
          this.logger.debug(
            `BATCH event - TYPE_OF_CENTER queried for parent cohort ${parentCohortId}: ${typeOfCenter}`
          );
        } else {
          this.logger.warn(
            `BATCH event missing parentId for cohort ${cohortId}`
          );
        }
      }
      ...
      if (cohortType && (cohortType === "COHORT" || cohortType === "BATCH")) {
        ...
        transformedCohortData.type = this.transformCohortType(
          cohortType as "COHORT" | "BATCH",
          typeOfCenter
        );
        this.logger.log(
          `Cohort type transformation - Original: ${originalType}, TYPE_OF_CENTER: ${typeOfCenter}, Transformed: ${transformedCohortData.type}`
        );
      } else {
        this.logger.warn(
          `Unknown cohort type: ${cohortType}, keeping original type`
        );
      }
    } catch (error) {
      // Keep original cohort type if transformation fails
      this.logger.error(
        `Failed to transform cohort type for cohort ${cohortId}: ${error.message}`
      );
    }
    ...
    this.logger.log(
      `Cohort ${eventType} event published for cohort ${cohortId} with type: ${transformedCohortData.type}`
    );
  }
  ...
  async publishCohortMemberEvent(
    ...
    if (!this.isKafkaEnabled) {
      this.logger.warn(
        "Kafka is disabled. Skipping cohort member event publish."
      );
      return;
    }
    ...
    this.logger.log(
      `Cohort member ${eventType} event published for cohortMembershipId ${cohortMembershipId}`
    );
  }
}
