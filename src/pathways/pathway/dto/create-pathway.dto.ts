...
export class CreatePathwayDto {
  ...
  @ApiPropertyOptional({
    description:
      "Image URL (from presigned S3 upload). upload to S3, then send the returned fileUrl here.",
  })
  @Expose()
  @IsOptional()
  @IsString()
  ...
  @ApiPropertyOptional({
    description:
      "Display order for sorting pathways (auto-incremented if not provided)",
    example: 1,
    minimum: 0,
  })
  @Expose()
