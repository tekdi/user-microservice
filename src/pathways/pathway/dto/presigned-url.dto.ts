...
export class PathwayPresignedUrlDto {
  @ApiProperty({
    description:
      "Image file name only (no path). Backend builds full key from PATHWAY_STORAGE_KEY_PREFIX env (e.g. pathway-images/pathway/files) + this name.",
  ...
  @ApiProperty({
    description:
      "Content-Type of the file. Allowed: image/jpeg, image/jpg, image/png, image/svg+xml",
    example: "image/png",
  })
  @IsString()
  @IsNotEmpty()
  @Matches(PATHWAY_IMAGE_MIME_REGEX, {
    message:
      "contentType must be one of: image/jpeg, image/jpg, image/png, image/svg+xml",
  })
  contentType: string;
  ...
  @ApiPropertyOptional({
    description:
      "Max file size in bytes (capped at 5 MB). Used for presigned POST content-length-range.",
    example: 5242880,
