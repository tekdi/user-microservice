...
export class SsoService {
  ...
  constructor(
    ...
    this.newtonApiTimeout =
      this.configService.get<number>("NEWTON_API_TIMEOUT") || 30000;
  }
  ...
  async authenticate(
    ...
    try {
      this.logger.log(
        `Starting SSO authentication for provider: ${ssoRequestDto.ssoProvider}`,
        "SSO_SERVICE"
      );
      ...
      if (!ssoRequestDto.tenantId) {
        ...
        this.logger.log(
          `Using default tenant ID: ${SSO_DEFAULTS.DEFAULT_TENANT_ID}`,
          "SSO_SERVICE"
        );
      }
      ...
      if (!ssoRequestDto.roleId) {
        ...
        this.logger.log(
          `Using default learner role ID: ${SSO_DEFAULTS.DEFAULT_LEARNER_ROLE_ID}`,
          "SSO_SERVICE"
        );
      }
      ...
      if (!roleName) {
        throw new HttpException(
          "Role name not found. Please enter a valid Role ID",
          HttpStatus.NOT_FOUND
        );
      }
      this.logger.log(
        `Role name fetched for roleId ${ssoRequestDto.roleId}: ${roleName}`,
        "SSO_SERVICE"
      );
      ...
      if (!newtonResponse.isNewUser) {
        this.logger.log(
          `Existing user detected: ${newtonResponse.name}`,
          "SSO_SERVICE"
        );
        ...
        this.handleExistingUser(newtonResponse, ssoRequestDto).catch(
          (error) => {
        ...
        this.logger.log(
          `New user detected: ${newtonResponse.name}`,
          "SSO_SERVICE"
        );
  ...
  private async getRoleNameDirectly(roleId: string): Promise<string> {
    try {
      ...
      const response = await this.roleService["roleRepository"].query(query, [
        roleId,
      ]);
      ...
      this.logger.error(
        `Failed to fetch role name for roleId ${roleId}: ${error.message}`,
        "SSO_SERVICE"
      );
      return "";
    }
   ...
   */
  private async callNewtonApi(
    ssoRequestDto: SsoRequestDto,
    roleName: string
  ): Promise<NewtonApiResponse> {
    try {
      this.logger.log(
        `Calling Newton API: ${this.newtonApiEndpoint}`,
        "SSO_SERVICE"
      );
      ...
      this.logger.log(
        `Form data being sent to Newton API: userId=${ssoRequestDto.userId}, roles=${roleName}`,
        "SSO_SERVICE"
      );
      ...
      this.logger.log(
        `Newton API response received successfully`,
        "SSO_SERVICE"
      );
      ...
      if (error.response) {
        ...
        this.logger.error(
          `Newton API HTTP Error Details:`,
          JSON.stringify(errorDetails),
          "SSO_SERVICE"
        );

        throw new HttpException(
          `Newton API HTTP Error [${error.response.status}]: ${
        ...
        this.logger.error(
          `Newton API Network Error: No response received`,
          "SSO_SERVICE"
        );
        throw new HttpException(
  ...
  private async handleNewUser(
    ...
    try {
      ...
      const userCreateDto = this.mapToUserCreateDto(
        newtonResponse,
        ssoRequestDto
      );
      ...
      if (
        ...
        Object.keys(newtonResponse.newtonData).length > 0
      ) {
        for (const [fieldLabel, fieldValue] of Object.entries(
          newtonResponse.newtonData
        )) {
          if (fieldValue) {
            const fieldId = await this.fieldsService.getFieldIdByLabel(
              fieldLabel,
              ssoRequestDto.tenantId
            );
      ...
      this.userService.publishUserEvent(
        "created",
        createdUser.userId,
        "api.sso.service"
      );
      this.logger.log(
        `New user created successfully: ${newtonResponse.name} with ID: ${createdUser.userId}`,
        "SSO_SERVICE"
      );
      this.userService.publishUserEvent(
        "created",
  ...
  private async updateNewtonDataFields(
    ...
    try {
      ...
      if (!newtonData || Object.keys(newtonData).length === 0) {
        this.logger.log(
          `No newtonData to update for user: ${userId}`,
          "SSO_SERVICE"
        );
        return;
      }
  ...
  private async handleExistingUser(
    ...
    try {
      ...
      if (!userId) {
        this.logger.error(
          "No userId available for existing user",
          "SSO_SERVICE"
        );
        throw new HttpException(
      ...
      this.logger.log(`Processing existing user: ${userId})`, "SSO_SERVICE");
      ...
      this.userService
        .publishUserEvent("updated", userId, "api.sso.service")
        .catch((error) => {
