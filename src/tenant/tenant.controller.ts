...
export class TenantController {
  ...
  public async createTenants(
    ...
    if (files && files.length > 0) {
      ...
      tenantCreateDto.programImages = uploadedFiles.map(
        (file) => file.filePath
  ...
  public async updateTenants(
    ...
    if (files && files.length > 0) {
      ...
      // Assuming tenantCreateDto needs an array of file paths
      tenantUpdateDto.programImages = uploadedFiles.map(
        (file) => file.filePath
      ); // Adjust field as needed
    }
  ...
  public async getTenantsById(
    @Req() request: Request,
    @Res() response: Response
  ): Promise<Response> {
    return await this.tenantService.getTenantsById(request, response);
  }
}
